...
(
'
SELECT prod_date, size_stand, COUNT(chick_no)::NUMERIC
FROM fms.prod_result
GROUP BY prod_date, size_stand
ORDER BY prod_date, size_stand
',
'
SELECT DISTINCT size_stand FROM fms.prod_result WHERE size_stand IN (10, 11, 12) ORDER BY 1
'
)
AS pivot_r(prod_date DATE, "10" NUMERIC, "11" NUMERIC, "12" NUMERIC);
